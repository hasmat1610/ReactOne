  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    if (session?.access_token && user) {
      setAuthContext(session.access_token, {
        name: (user.user_metadata as { name?: string })?.name ?? user.email ?? undefined,
    ...
    <div className="min-h-[100dvh] bg-[#070707] flex flex-col items-center justify-center p-4 selection:bg-primary/30 antialiased font-sans relative z-10">
      <div className="w-full max-w-[360px] flex flex-col items-center">
        ...
        <p className="text-sm text-neutral-400 mb-8 font-medium">
          Sign in to access your components
        </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="text-sm text-neutral-400 font-medium" htmlFor="password">
                Password
              </label>
              <Link
                to="/forgot-password"
                className="text-xs text-primary hover:text-sky-200 transition-colors"
              >
                Forgot password?
              </Link>
            </div>
